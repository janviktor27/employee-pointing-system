...
//Add employee
	function add(){
		...
		if(isset($_POST['btn_add'])){
			...
			if($count > 0){
				ob_end_clean();
				header("location: co-clerks.php?add=exist");
			}else{
				$sqlInsert = mysqli_query($_CON, "INSERT INTO employee_table 
				(EMP_EIN, EMP_FNAME, EMP_LNAME, USER_TYPE, EMP_PASSWORD)
				VALUES('$inputEIN','$inputFNAME','$inputLNAME','1','$default_pass')");
				ob_end_clean();
				header("location: co-clerks.php?add=true");
			}
		}
	}
...
//View employee
	function theview(){
		global $_CON;
		$sqlSearch = mysqli_query($_CON, "SELECT * FROM employee_table WHERE USER_TYPE='1' ");
		$count = mysqli_num_rows($sqlSearch);
		if($count > 0){
	...
	function updAction(){
		...
		if(isset($_POST['btn_upd'])){
			...
			if($inputEIN == $_GETEIN){
				...
				EMP_FNAME='$inputFNAME', EMP_LNAME='$inputLNAME' WHERE EMP_ID='$_ID' ");
				ob_end_clean();
				header("location: co-clerks.php?upd=true");
			}else{
				//CHECK IF EIN EXIST
				$sqlSearch = mysqli_query($_CON, "SELECT EMP_EIN FROM employee_table WHERE EMP_EIN='$inputEIN' ");
				$count = mysqli_num_rows($sqlSearch);
				if($count == 1){
					ob_end_clean();
					header("location: co-clerks.php?upd=exist");
				}else{
					$sqlUpdate = mysqli_query($_CON,"UPDATE employee_table SET
					EMP_EIN='$inputEIN', EMP_FNAME='$inputFNAME', EMP_LNAME='$inputLNAME' WHERE EMP_ID='$_ID' ");
					ob_end_clean();
					header("location: co-clerks.php?upd=true");
				}
			}
...
//DELETE MODAL
	function delAction(){
		...
		if(isset($_POST['btn_del'])){
			...
			if($sqlDel){
				ob_end_clean();
				header("location: co-clerks.php?del=true");
			}else{
				ob_end_clean();
				header("location: co-clerks.php?del=false");
			}
		}
	}
